...
from models.ai_model import evaluate_answers_with_ai
...
def start_interview(job_title):
    ...
    if request.method == 'POST':
        ...
        answers = [request.form.get(f"answer_{idx + 1}", "").strip() for idx in range(len(questions))]
        answers = [answer if answer else "No answer provided." for answer in answers]

        # Debug: Print questions and answers
        print("Questions:", questions)  # Debug statement
        print("Answers:", answers)      # Debug statement

        # Evaluate answers using the AI model
        evaluation_results = evaluate_answers_with_ai(answers, questions)

        # Debug: Print AI evaluation results
        print("Evaluation Results:", evaluation_results)  # Debug statement
